...
export const getEmployeeProfileById = async (emp_table_id: number) => {
    ...
    const result = await pool.query(`SELECT * FROM employees WHERE id = $1`, [emp_table_id]);
    return result.rows[0];
};
...
export const getEmployeeTotalLeaves = async (emp_id: number, thisMonth: boolean) => {
    if (thisMonth) {
        ...
        const result = await pool.query(`SELECT * FROM leaves WHERE emp_id = $1`, [emp_id]);
...
export const deleteEmployeeByEmpId = async (emp_id: string) => {
    const result = await pool.query(`DELETE FROM employees WHERE emp_id = $1 RETURNING *`, [
        emp_id,
    ]);
    return result.rows[0];
};
...
export const getEmployeeTotalPresent = async (emp_id: string, thisMonth: boolean) => {
    if (thisMonth) {
        ...
        const result = await pool.query(`SELECT * FROM attendance WHERE emp_id = $1`, [emp_id]);
...
export const getEmployeeBasicDetails = async () => {
    const result = await pool.query(
        'SELECT id, emp_id, email, first_name, last_name FROM employees'
    );
