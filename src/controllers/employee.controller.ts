...
import {
    ...
    getEmployeeTotalLeaves,
    getEmployeeTotalPresent,
...
export const login = async (req: Request, res: Response) => {
    ...
    try {
        const token = jwt.sign(payload, Buffer.from(jwtSecret), {
            algorithm: 'HS256',
            expiresIn: '2h',
        });
...
// Get employees basic details
export const getEmployeeBasicDetailsController = async (req: Request, res: Response) => {
    try {
        const employeeData = await getEmployeeBasicDetails();
        res.status(200).json(employeeData);
    } catch (error) {
        console.log('Error while getting employees basic details', error);
        res.status(500).json({ message: 'Error getting employee basic details.' });
    }
};
